...
gridEZ <- function(population_raster, settlement_raster, strata_raster,
  ...
  if (sp::proj4string(population_raster) != "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0") {
    population_raster <- projectRaster(
      population_raster,
  ...
  if (sp::proj4string(settlement_raster) != "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0") {
    settlement_raster <- projectRaster(
      settlement_raster,
      crs = sp::CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"),
      method = "ngb")
  }
 ...
 # browser()
  ...
  strat_sett_df <- cbind(
    rep(strat_vals, each = length(sett_vals)),
    rep(sett_vals, length(strat_vals)))
  clump_info <- list()
  ...
  for(ssID in 1:nrow(strat_sett_df)){
    ...
    if (!any(class(sub_strat_sett_pop_mat) == "matrix")){sub_strat_sett_pop_mat <- matrix(sub_strat_sett_pop_mat, nrow=(max_strat_sett_row - min_strat_sett_row + 1))}
    ...
    for(block_size in blocks){
      ...
      for(clump_ID in clump_IDs){
        ...
        # Added any() to class checks below 2023-09-07 (CBC)
        subclump_mat <- clump_mat[min_clump_row:max_clump_row, min_clump_col:max_clump_col]
        if (!any(class(subclump_mat) == "matrix")){subclump_mat <- matrix(subclump_mat, nrow=(max_clump_row - min_clump_row + 1))}
        ...
        if (!any(class(subpop_mat) == "matrix")){subpop_mat <- matrix(subpop_mat, nrow=(max_clump_row - min_clump_row + 1))}
...
EZ_gen_fn <- function(clump_x, max_cells_per_EZ, output_path, run_ID){
  ...
  # Added any() 2023-09-08
  if (any(class(subpop_mat) == "matrix")){                       #i.e. more than 1 cell (almost always)
  ...
  # If total pop of clump is smaller than min pop per EZ AND total number of
  # cells in clump (given by nrow(cell_IDs)) is smaller than max cells per EZ,
  # then don't split into multiple EZs & save clump as single EZ
  ...
  if (((clump_pop_size <= max_pop_per_EZ) & (nrow(cell_IDs) <= max_cells_per_EZ * 1.5)) | nrow(cell_IDs)==1){
      ...
      repeat{
        ...
        compact_vals <-  lapply(test_splits,function(test_EZ_split){
          ...
          for(x in 1:2){
            ...
            # Added any() 2023-09-08
            if (!any(class(test_EZ_locs) == "matrix")){test_EZ_locs <- matrix(test_EZ_locs, nrow=1)}
    ...
    for(EZ_ID in EZ_ID_vec){
      ...
      # Added any() 2023-09-08
      if (any(class(subEZ_IDs_mat) == "matrix")){
